...
let {event} = require('../../saml-redirect/saml-request.json');
const request = require('supertest');

const elig_rule = { productTypes: ["BG", "BP"], validationFields: ["reseller_employee_id", "role", "dob"] }
const EMPLOYER = { id: 23, reseller_id: 1, external_id: '20005', eligibility_rules: elig_rule };
const RESELLER = { 
    id: 1, eid: '1W85pu', name: 'Virgin Pulse', description:"Virgin Pulse", 
    eligibility_rules: {"productTypes": ["BP","BG","WM","MSK"],validationFields: ["eid"]},
    configurations: '{"sso":{"cert_filename":"certs/prod-VigrinPlus-metadata.txt"}}'
}
const ELIGIBILITY = { 
...
describe('Test for api-gw-saml-redirect-handler', () => {
    ...
    it('Verifies <Virgin Pulse SAML response> - redirect correctly', async() => {
        ...
        let response = await request(apiSAMLRedirectHandler.express_app).post('/reseller/1W85pu/saml').send(event.body);
        //console.warn(JSON.stringify(response));
        expect(response.status).toEqual(303);
        expect(response.header.location).toEqual(process.env.EligiblityLandingPageHost + "/emp/20005?auth=ZGFyaW90ZXN0c3VpdGUZGFyaW90ZXN0c3VpdGUZGFyaW90ZXN0c3VpdGUZGFy");
    });    
